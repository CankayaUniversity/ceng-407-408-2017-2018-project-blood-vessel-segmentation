function [positiveCounter,negativeCounter] = pickPatchesTest(image,groundTruth,strideP,strideN,patchSize,p,n)
    ...
    for j = 1:strideP:n-(patchSize-1)
        ...
        if mean2(patch) > 20 
            ...
            if patchClasifier(GTpatch)
                insertPositiveTest(patch,positiveCounter),positiveCounter=positiveCounter+1;
            end
        end
    end
end
    ...
    for j = 1:strideN:n-(patchSize-1)
        ...
        if mean2(patch) > 20 
            ...
            if patchClasifier(GTpatch)==false
                insertNegativeTest(patch,negativeCounter),negativeCounter=negativeCounter+1;
            end
        end
    end
end
